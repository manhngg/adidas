class TransceiverGroupsController < BaseController
  ...
  def update
    respond_to do |format|
      if @transceiver_group.update(transceiver_group_params)
        format.html { redirect_to groups_url(store_id: @store.id), notice: t("flash.update", name: "トランシーバグループ") }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to groups_url(store_id: @store.id), notice: t("flash.destroy", name: "トランシーバグループ") }
    end
  end
