class Api::V1::InventoriesController < ApplicationController
  ...
  def update_product_inventory
    ...
    if @list_warning.blank? && @list_error.blank?
      render json: {status: 200}, status: 200
    else
      if @list_error.blank? && @list_warning.present?
        render json: {status: 200, error: @list_warning}, status: 200
      else
  ...
  def update_item_product_inventory item
    ...
    unless store
      @list_error << "Row #{item[:row]} and col #{item[:col]}: 店舗が見つかりません"
      return false
    end
    product_inventory = store.product_inventories.find_by_jan_code item[:jan_code]
    unless product_inventory
      @list_warning << "Row #{item[:row]} and col #{item[:col]}: 商品在庫が見つかりません"
      return false
    end
    product = store.products.find_by_jan_code item[:jan_code]
    unless product
      @list_warning << "Row #{item[:row]} and col #{item[:col]}: 商品が見つかりません"
      false
    end
