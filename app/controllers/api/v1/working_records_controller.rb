class Api::V1::WorkingRecordsController < ApplicationController
  ...
  def create_or_update_record_report
    @record_report = user.record_reports.find_by store_id: params[:store_id],
  ...
  def create_record_report_with_params args = {}
    @record_report = user.record_reports.create! record_date: Date.today,
  ...
  def update_record_report
    #出勤
    if working_record.record_action_id == 1
      user.record_reports.update record_date: Date.today,
        start_time: Time.zone.now, store_id: params[:store_id]
    end
    ...
    if working_record.record_action_id == 3
      ...
      return unless working_record_previous
      user.record_reports.update record_date: Date.today,
        store_id: params[:store_id], break_time:
        (Time.zone.now - working_record_previous.action_time).to_i
    end
    ...
    if working_record.record_action_id == 4 #退勤
      user.record_reports.update record_date: Date.today,
        stop_time: Time.zone.now, store_id: params[:store_id]
    end
  end
