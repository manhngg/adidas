class UsersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @user.save
        ...
        format.html {redirect_to users_url(store_id: @store.id), notice: 'User was successfully created.'}
      else
  ...
  def update
    respond_to do |format|
      if @user.update(user_params)
        format.html {redirect_to users_url(store_id: @store.id), notice: 'User was successfully updated.'}
      else
  ...
  def destroy
    @user.destroy
    respond_to do |format|
      format.html {redirect_to users_url(store_id: @store.id), notice: 'User was successfully destroyed.'}
    end
  end
  ...
  def csv_import
    if params[:file].present? && params[:file].original_filename &&
      ...
      redirect_to import_users_url(store_id: @store.id), alert: "インポートするCSVファイルを選択してください"
    end
  end
