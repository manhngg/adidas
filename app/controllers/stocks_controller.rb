class StocksController < BaseController
  ...
  def create
    ...
    respond_to do |format|
      if @stock.save
        format.html { redirect_to stocks_url(store_id: @store.id), notice: t("flash.create", name: "在庫") }
      else
  ...
  def update
    respond_to do |format|
      if @stock.update(stock_params)
        format.html { redirect_to stocks_url(store_id: @store.id), notice: t("flash.update", name: "在庫") }
      else
  ...
  def destroy
    @stock.destroy
    respond_to do |format|
      format.html { redirect_to stocks_url(store_id: @store.id), notice: t("flash.destroy", name: "在庫") }
    end
  end
