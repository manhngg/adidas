class OrdersController < BaseController
  ...
  def create
    ...
    respond_to do |format|
      if @order.save
        format.html { redirect_to orders_url(store_id: @store.id), notice: t("flash.create", name: "発注") }
      else
  ...
  def update
    respond_to do |format|
      if @order.update hash_order_params
        format.html { redirect_to orders_url(store_id: @store.id), notice: t("flash.update", name: "発注") }
      else
  ...
  def destroy
    @order.destroy
    respond_to do |format|
      format.html { redirect_to orders_url(store_id: @store.id), notice: t("flash.destroy", name: "発注") }
    end
  end
  ...
  def csv_import
    if params[:file].present? && params[:file].original_filename &&
      ...
      redirect_to import_orders_url(store_id: @store.id)
      flash[:notice] = import.values.first
      flash[:alert] = import.values.reject{|value| value == import.values.first}
    else
      redirect_to import_orders_url(store_id: @store.id), notice: t("flash.csv_not_found")
    end
  end
