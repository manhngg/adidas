...
RSpec.describe Api::V1::InventoriesController, type: :controller do
  describe "GET /api/v1/inventories" do
    ...
    it "request has status 200" do
      get "list_shelf", format: :json, params: {store_id: store.id}
      expect(response.status).to eq 200
    end

    it "request has status 500" do
      get "list_shelf", format: :json, params: {store_id: store_new.id}
      expect(response.status).to eq 500
    end

    it "request has status 404" do
      get "list_shelf", format: :json, params: {store_id: nil}
  ...
  describe "POST /api/v1/inventories" do
    ...
    before do
      @token = JsonWebToken.encode(api_key: FactoryBot.create(:api_key, store_id: store_first.id).key_value)
      request.headers.merge!({'Authorization': @token})
    end
